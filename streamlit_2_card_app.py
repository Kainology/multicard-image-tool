...
def merge_two_images(urls, output_name):
...
st.title("🖼️ Ghép 2 ảnh MULTICARD thành 1 ảnh 410x336")
...
if uploaded_file:
    ...
    if len(df) % 2 != 0:
        st.error("❌ Số dòng trong file CSV phải chia hết cho 2.")
    elif df.shape[1] < 2:
        ...
        for i in range(0, len(df), 2):
            ...
            try:
                result_file = merge_two_images(urls, name)
        ...
        # Nén kết quả
        if result_files:
            zip_filename = "merged_images_2cards.zip"
