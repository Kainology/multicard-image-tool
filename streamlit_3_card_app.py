...
def merge_three_images(urls, output_name):
...
st.title("🖼️ Ghép 3 ảnh MULTICARD thành 1 ảnh 410x336")
...
if uploaded_file:
    ...
    if len(df) % 3 != 0:
        st.error("❌ Số dòng trong file CSV phải chia hết cho 3.")
    elif df.shape[1] < 2:
        ...
        for i in range(0, len(df), 3):
            ...
            try:
                result_file = merge_three_images(urls, name)
        ...
        # Nén kết quả
        if result_files:
            zip_filename = "merged_images_3cards.zip"
